...
static void	print_map_grid(t_map *map)
{
	...
	printf("\n");
	printf("Legend: █=Wall  ·=Floor  ↑↓←→=Player\n");
	printf("\n");
	print_separator();
}
...
static void	print_summary(t_map *map)
{
	...
	print_separator();
	printf("\n");
	printf("✅ All 4 wall textures loaded\n");
	printf("✅ Floor and ceiling colors configured\n");
	printf("✅ Map parsed: %d x %d (%d total cells)\n", 
...
static void	print_usage(const char *program)
{
	...
	printf("  ✓ Hidden files (starting with .) are not allowed\n");
	printf("  ✓ File cannot be named just '.cub'\n");
	printf("\n");
...
int	main(int argc, char **argv)
{
	...
	if (!parse_cub_file(argv[1], &config, &map))
	{
		...
		printf("  • All required identifiers are present (NO, SO, EA, WE, F, C)\n");
		printf("  • Texture files exist and are accessible\n");
		printf("  • Colors are in valid RGB format (0-255)\n");
		printf("  • The map is properly formatted\n");
		printf("  • There is exactly one player position (N/S/E/W)\n");
		printf("\n");
		return (1);
	}

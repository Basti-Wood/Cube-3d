...
{
	if (game->texture_path[FLOOR])
	{
		printf("Error: Duplicate NO identifier\n");
		return (0);
	}
	return (parse_texture(value, &game->texture_path[FLOOR]));
}

static int	handle_east_texture(char *value, t_game *game)
{
	if (game->texture_path[DOOR])
	{
		printf("Error: Duplicate EA identifier\n");
		return (0);
	}
	return (parse_texture(value, &game->texture_path[DOOR]));
}

static int	handle_south_texture(char *value, t_game *game)
{
	if (game->texture_path[WALL])
	{
		printf("Error: Duplicate SO identifier\n");
		return (0);
	}
	return (parse_texture(value, &game->texture_path[WALL]));
}

static int	handle_west_texture(char *value, t_game *game)
{
	if (game->texture_path[CEILING])
	{
		printf("Error: Duplicate WE identifier\n");
		return (0);
	}
	return (parse_texture(value, &game->texture_path[CEILING]));
}
